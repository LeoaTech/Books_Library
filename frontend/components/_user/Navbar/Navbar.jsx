...
const Navbar = () => {
  ...
  return (
    <header className="w-full bg-white fixed top-0 left-0 right-0 transition-all ease-in duration-300 z-50">
      {/* Left */}
      <nav
        className={`py-6 lg:px-24 px-4 z-50 ${isSticky ? "sticky top-0 left-0 right-0 bg-purple-300 shadow-lg" : ""
          }`}
      >
        <div className="flex justify-between items-center text-base gap-8">
          ...
          <div className="text-sm font-medium text-center text-gray-500 border-b border-gray-200 ">
            <ul className="flex flex-wrap -mb-px md:flex space-x-8 hidden">
              <li className="mr-2">
                {headerLinks?.map(({ title, path }) => (
                  <Link
                    to={path}
                    key={path}
                    className="inline-block text-base text-green-900 uppercase cursor-pointer p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-red-500 "
                  >
                    {title}
                  </Link>
                ))}
          ...
          <div
            className={`${isMenuOpen ? "hidden" : "hidden lg:flex gap-4 items-center"
              }`}
          >
            <Link
              className="block text-base text-blue-900 uppercase cursor-pointer hover:text-red-400"
              to="/cart"
            >
              <HiOutlineShoppingCart className="h-5 w-5 " />{" "}
            </Link>
            <SignedOut>
              <Link to="/sign-in" className="block text-base text-blue-900 uppercase cursor-pointer hover:text-red-400">
                Sign in
              </Link>
              <Link to="/sign-up" className="h-9 flex justify-center items-center px-4 ml-auto text-sm bg-blue-500 text-white rounded-lg text-white-900  text-center px-2 py-1 uppercase cursor-pointer hover:text-white-400 hover:bg-blue-300">Sign up
              </Link>
            </SignedOut>
        ...
        {/* Mobile Screen menu */}
        <div
          className={`space-y-8 px-8 mt-16 py-4 bg-slate-600 text-black md:hidden ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"
            } `}
        >
          {headerLinks?.map(({ title, path }) => (
            <Link
          ...
          <div onClick={onToggle} className={`${isMenuOpen ? "sm:hidden" : "hidden lg:flex gap-4 items-center"
            }`}>
            <Link
            ...
            <SignedOut>
              <Link to="/sign-in" className="h-9 flex justify-center items-center px-4 ml-auto text-sm bg-transparent text-black rounded-lg   text-center px-2 py-1 uppercase cursor-pointer hover:bg-black hover:text-white">
                Sign in
              </Link>
              <Link to="/sign-up" className="h-9 flex justify-center items-center px-4 ml-auto text-sm bg-blue-500 text-white rounded-lg text-white-900  text-center px-2 py-1 uppercase cursor-pointer hover:text-white-400 hover:bg-blue-300">
                Sign up
              </Link>
            </SignedOut>
            {user && <a onClick={signOut} className="h-9 mt-2 flex justify-center items-center px-4 ml-auto text-sm bg-blue-500 text-white rounded-lg text-white-900  text-center px-2 py-1 uppercase cursor-pointer hover:text-white-400 hover:bg-blue-300">Sign Out</a>}
